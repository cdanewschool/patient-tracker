...
function FHIRAdapter()
{
	this.LOINC_CODE_BLOOD_PRESSURE = "55284-4";
	this.LOINC_CODE_BLOOD_PRESSURE_DIASTOLIC = "8462-4";
	this.LOINC_CODE_BLOOD_PRESSURE_SYSTOLIC = "8480-6";
	this.LOINC_CODE_BODY_HEIGHT = "3137-7";
	this.LOINC_CODE_HEART_RATE = "8867-4";
	this.LOINC_CODE_RESPIRATORY_RATE = "9279-1";
	this.LOINC_CODE_BODY_TEMPERATURE = "8310-5";
	this.LOINC_CODE_WEIGHT ="3141-9";
	 ...
	 * Medication
	 */
	this.parseMedicationStatements = function( data )
	{
		...
		for(var i=0;i<entries.length;i++)
		{
		    ...
		    m.frequencyValue=data.dosage.timing.repeat.frequency.value;
		    m.frequencyUnits=data.dosage.timing.repeat.units.value;
		    m.routeCode=data.dosage.route.coding[0].code.value;
		    m.routeValue=data.dosage.route.coding[0].display.value;
		    m.endDate=data.dosage.timing.event?data.dosage.timing.event.end:null;
	...
	this.parseMedicationAdministrations = function( data )
    {
    ...
    this.getMedicationStatement = function ( patientId, medicationObj, dateStringStart, dateStringEnd, dosageRoute, dosageQuantity, dosageUnits, dosageRepeatFrequency, dosageRepeatUnits )
	{
		...
		dosage.quantity = new Quantity(new Value(dosageQuantity), new Value(dosageUnits.value), new Value(this.constants.UNITS_URL), new Code(dosageUnits.value) );
		dosage.timing = new Schedule( period, {frequency:new Code(dosageRepeatFrequency),units:new Code(dosageRepeatUnits)} );
	    
		if( dosageRoute )
            dosage.route = new CodeableConcept([new Coding(new Value(this.constants.SNOMED_URL),new Code(dosageRoute.value),new Value(dosageRoute.label))] );
	...
	this.getMedicationAdministration = function ( patientId, practitionerId, medicationObj, dateStringStart, dateStringEnd )
	{
		...
		var medication = new ResourceReference( new Value(medicationObj.name), new Value("medication/@" + medicationObj.medicationId), new Code("Medication") );
		
		var dosage = new MedicationDosage();
		dosage.quantity = new Quantity(new Value(medicationObj.dosageValue), new Value(medicationObj.dosageUnits), new Value(this.constants.UNITS_URL), new Code(medicationObj.dosageUnits) );
		dosage.route = new CodeableConcept([new Coding(new Value(this.constants.SNOMED_URL),new Code(medicationObj.routeCode),new Value(medicationObj.routeValue))] );
		//TODO: right to omit frequency/units when recording a single event?
		dosage.timing = new Schedule( period/*, {frequency:new Code(dosageRepeatFrequency),units:new Code(dosageRepeatUnits)}*/ );
     ...
     * Vitals
     */
	this.parseVitalStatements = function( data )
	{
		var items = [];
	...
	this.getVitalStatement = function(patientId,name,code,codeDisplay,codeSystem)
	{
		var tracker = {};
	...
	this.parseVital = function ( data )
	{
		...
		vital.reportedBy = this.constants.REPORTER_PATIENT;
		...
		if( code == this.LOINC_CODE_BLOOD_PRESSURE_DIASTOLIC )
			vital.type = this.constants.VITAL_TYPE_BLOOD_PRESSURE_DIASTOLIC;
		else if( code == this.LOINC_CODE_BODY_HEIGHT )		//	body height measured
			vital.type = this.constants.VITAL_TYPE_HEIGHT;
		else if( code == this.LOINC_CODE_HEART_RATE )
			vital.type = this.constants.VITAL_TYPE_HEART_RATE;
		else if( code == this.LOINC_CODE_RESPIRATORY_RATE )
			vital.type = this.constants.VITAL_TYPE_RESPIRATORY_RATE;
		else if( code == this.LOINC_CODE_BLOOD_PRESSURE_SYSTOLIC )
			vital.type = this.constants.VITAL_TYPE_BLOOD_PRESSURE_SYSTOLIC;
		else if( code == this.LOINC_CODE_BODY_TEMPERATURE )
			vital.type = this.constants.VITAL_TYPE_BODY_TEMPERATURE;
		else if( code == this.LOINC_CODE_WEIGHT )		//	body weight measured
			vital.type = this.constants.VITAL_TYPE_WEIGHT;
		
		return vital;
	};
	
	this.getVital = function ( id, value, value2, units, patientId, dateString )
	{
		var observation = {};
		...
		observation.interpretation = new CodeableConcept( [new Coding(new Value(this.constants.HL7_URL + "v2/0078"),new Code("N"),new Value("Normal (applies to non-numeric results)"))] );
		
		var issueDateString = this.constants.MONTHS_ABBR[observation.issued.value.getMonth()] + " " + observation.issued.value.getDate() + " " + observation.issued.value.getFullYear();
		
		if( id == this.constants.VITAL_TYPE_BLOOD_PRESSURE )
		{
			var systolic = new ObservationComponentComponent();
			systolic.name = new CodeableConcept( [new Coding(new Value(this.constants.LOINC_URL),new Code(this.LOINC_CODE_BLOOD_PRESSURE_SYSTOLIC),new Value("Systolic blood pressure"))] );
			systolic.valueQuantity = new Quantity( new Value(value), new Value(units) );
			
			var diastolic = new ObservationComponentComponent();
			diastolic.name = new CodeableConcept( [new Coding(new Value(this.constants.LOINC_URL),new Code(this.LOINC_CODE_BLOOD_PRESSURE_DIASTOLIC),new Value("Diastolic blood pressure"))] );
			diastolic.valueQuantity = new Quantity( new Value(value2), new Value(units) );
			
			observation.name = new CodeableConcept( [new Coding(new Value(this.constants.LOINC_URL),new Code(this.LOINC_CODE_BLOOD_PRESSURE),new Value("Blood pressure systolic and diastolic"))] );
		...
		else if( id == this.constants.VITAL_TYPE_HEART_RATE )
		{
			var heartRate = new ObservationComponentComponent();
			heartRate.name = new CodeableConcept( [new Coding(new Value(this.constants.LOINC_URL),new Code(this.LOINC_CODE_HEART_RATE),new Value("Heart rate"))] );
			heartRate.valueQuantity = new Quantity( new Value(value), new Value(units) );
			
			observation.name = new CodeableConcept( [new Coding(new Value(this.constants.LOINC_URL),new Code(this.LOINC_CODE_HEART_RATE),new Value("Heart rate"))] );
		...
		else if( id == this.constants.VITAL_TYPE_RESPIRATORY_RATE )
		{
			var respiratoryRate = new ObservationComponentComponent();
			respiratoryRate.name = new CodeableConcept( [new Coding(new Value(this.constants.LOINC_URL),new Code(this.LOINC_CODE_RESPIRATORY_RATE),new Value("Respiratory rate"))] );
			respiratoryRate.valueQuantity = new Quantity( new Value(value), new Value(units) );
			
			observation.name = new CodeableConcept( [new Coding(new Value(this.constants.LOINC_URL),new Code(this.LOINC_CODE_RESPIRATORY_RATE),new Value("Respiratory rate"))] );
		...
		else if( id == this.constants.VITAL_TYPE_BODY_TEMPERATURE )
		{
			var temperature = new ObservationComponentComponent();
			temperature.name = new CodeableConcept( [new Coding(new Value(this.constants.LOINC_URL),new Code(this.LOINC_CODE_BODY_TEMPERATURE),new Value("Body temperature"))] );
			temperature.valueQuantity = new Quantity( new Value(value), new Value(units) );
			
			observation.name = new CodeableConcept( [new Coding(new Value(this.constants.LOINC_URL),new Code(this.LOINC_CODE_BODY_TEMPERATURE),new Value("Body temperature"))] );
		...
		else if( id == this.constants.VITAL_TYPE_WEIGHT )
		{
			var weight = new ObservationComponentComponent();
			weight.name = new CodeableConcept( [new Coding(new Value(this.constants.LOINC_URL),new Code(this.LOINC_CODE_WEIGHT),new Value("Weight"))] );
			weight.valueQuantity = new Quantity( new Value(value), new Value(units) );
			
			observation.name = new CodeableConcept( [new Coding(new Value(this.constants.LOINC_URL),new Code(this.LOINC_CODE_WEIGHT),new Value("Weight"))] );
	 ...
	 * Tracker
	 */
	this.parseTrackerStatements = function( data )
	{
		var items = [];
		
		console.log( 'parseTrackerStatements', data );
	...
	//	http://www.hl7.org/implement/standards/fhir/other.html
	this.getTrackerStatement = function(patientId,name,code,codeDisplay,codeSystem)
	{
		var tracker = {};
	...
	this.getTracker = function ( name, value, units, patientId, dateString, code, codeSystem )
	{
		var observation = {};
		...
		component.valueQuantity = new Quantity( new Value(value), new Value(units), new Value(this.constants.UNITS_URL), new Code(units) );
	 ...
	 * User
	 */
	this.getUser = function(username,password,name)
	{
	    var user = {};
